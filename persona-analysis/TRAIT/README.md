...
```
cd src/
​python run.py --model_name /scratch/manas/Qwen2.5-3B-Instruct --model_name_short qwen2.5-3B --inference_type chat --prompt_type 1
```
**Fine-tuned models:**
...
```
cd results/
python plot.py --model Qwen2.5-0.5B --num n <file1.txt> ... <filen.txt>
```
...
## Running persona-vector steered TRAIT

python run.py \
    --model_name "/scratch/manas/Qwen2.5-7B-Instruct" \
    --model_name_short "qwen2.5-7B_pv_psychopathy" \
    --prompt_type 1 \
    --inference_type "chat" \
    --coef 2.0 \
    --vector_path "../../persona_vectors/persona_vectors/Qwen2.5-7B-Instruct/psychopathy_response_avg_diff.pt" \
    --layer 20
