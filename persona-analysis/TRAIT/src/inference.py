...
def get_option_letter(model, tokenizer, prompt, device):
    # Generate only the option letter (A/B/C/D)
    ...
    letter = tokenizer.decode(outputs[0][inputs['input_ids'].shape[1]:], skip_special_tokens=True).strip()
    return letter

def get_full_answer(model, tokenizer, prompt, device):
    # Generate the full natural language answer
    inputs = tokenizer(prompt, return_tensors="pt").to(device)
    gen_config = GenerationConfig(
        max_new_tokens=100,
        do_sample=False,
...
def main():
    ...
    while True:
        ...
        # 1. Get the option letter (to match run.py's deterministic selection)
        letter = get_option_letter(model, tokenizer, user_input, device)
        print(f"Model (option letter): {letter}")

        # 2. Get the full natural language answer (for explanation)
        response = get_full_answer(model, tokenizer, user_input, device)
        print(f"Model (full answer): {response}\n")
