...
def main(model, trait, output_path, coef=0, vector_path=None, layer=None, steering_type="response", max_tokens=1000, n_per_question=10, batch_process=True, max_concurrent_judges=32, persona_instruction_type=None, assistant_name=None, judge_model="gpt-4.1-mini-2025-04-14", version="extract", overwrite=False):
    """
    Evaluates a model by first generating all responses, then releasing the model,
    and finally loading a separate judge model to score the responses.
    This sequential process is designed to work with two different large local models
    on limited VRAM.
    """
    ...
    # ===================================================================
    if coef != 0:
        # --- STEERING GENERATION (uses standard Hugging Face model) ---
        print(f"Loading HF Model ({model}) for steering...")
        llm, tokenizer = load_model(model)
        vector = torch.load(vector_path, weights_only=False)[layer]
        
        print(f"\n--- Generating STEERED answers (coef={coef}, layer={layer}) ---")
        
        all_paraphrases, all_conversations, question_ids = [], [], []
        for q in questions:
            paraphrases, conversations = q.get_input(n_per_question)
            all_paraphrases.extend(paraphrases)
            all_conversations.extend(conversations)
            question_ids.extend([q.id] * len(paraphrases))

        prompts, answers = sample_steering(llm, tokenizer, all_conversations, vector, layer, coef, temperature=questions[0].temperature, max_tokens=max_tokens, steering_type=steering_type)
        
        unjudged_df = pd.DataFrame({"question": all_paraphrases, "prompt": prompts, "answer": answers, "question_id": question_ids})
        del vector

    else:
        print(f"--- Step 1: Loading Generation Model ({model}) ---")
        llm, tokenizer, lora_path = load_vllm_model(model)

        print(f"--- Generating answers for persona_instruction_type='{persona_instruction_type}' ---")
    ...
    # ===================================================================
    print("\n--- Step 2: Releasing generation model from memory ---")
    del llm, tokenizer
    ...
    # ===================================================================
    print(f"\n--- Step 3: Loading Judge Model ({judge_model}) ---")
    is_openai_judge = judge_model.startswith("gpt-4") or judge_model.startswith("o4-")

    if is_openai_judge:
        raise NotImplementedError("Sequential script currently only supports local judges.")
    
    judge_llm, judge_tokenizer, _ = load_vllm_model(judge_model)
    
    trait_data = json.load(open(f"data_generation/trait_data_{version}/{trait}.json", "r"))
    judge_prompts_templates = { trait: trait_data["eval_prompt"], "coherence": Prompts[f"coherence_0_100"] }
    judges = { metric: VLLMJudge(judge_llm, judge_tokenizer, prompt) for metric, prompt in judge_prompts_templates.items() }
    
    # Prepare the DataFrame for new score columns
    for metric in judges.keys():
        unjudged_df[metric] = None

    tasks = [{'index': index, 'metric': metric, 'judge': judge, 'row': row} 
             for index, row in unjudged_df.iterrows() for metric, judge in judges.items()]

    with tqdm(total=len(tasks), desc="Judging") as pbar:
        for task in tasks:
            score = asyncio.run(task['judge'](question=task['row']['question'], answer=task['row']['answer']))
            unjudged_df.loc[task['index'], task['metric']] = score
            pbar.update(1)
    ...
    print(f"\nSaved final judged results to {output_path}")
    
    for t in [trait, "coherence"]:
        if t in outputs.columns and pd.to_numeric(outputs[t], errors='coerce').notna().any():
            mean_score = pd.to_numeric(outputs[t], errors='coerce').mean()
            std_score = pd.to_numeric(outputs[t], errors='coerce').std()
            print(f"{t}:  {mean_score:.2f} +- {std_score:.2f}")
