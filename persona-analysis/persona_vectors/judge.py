...
class VLLMJudge:
    """
    A judge that uses a local VLLM model to score responses.
    It works by generating a full text response and parsing a score
    from structured tags (e.g., <score>85</score>). This avoids
    the tokenizer-specific issues of logprob-based scoring.
    """
    def __init__(self, model: "LLM", tokenizer, prompt_template: str, eval_type: str = "0_100"):
        self.model = model
        ...
        self.sampling_params = SamplingParams(
            temperature=0,
            max_tokens=20, # Should be more than enough to get the score tag
            stop=["<|im_end|>", self.tokenizer.eos_token],
        )

    async def judge(self, **kwargs) -> float:
        """
        Formats the prompt, generates text, and parses the score.
        Returns the score as a float, or None if parsing fails.
        """
        prompt = self.prompt_template.format(**kwargs)
        messages = [dict(role='user', content=prompt)]
        full_prompt = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        
        # VLLM's generate is blocking, but we wrap it in an async method
        # to be compatible with the existing asyncio evaluation loop.
        completions = self.model.generate([full_prompt], self.sampling_params, use_tqdm=False)
        response_text = completions[0].outputs[0].text
        return self.parse_score(response_text)
...
class HFJudge:
    """ A judge that uses a local Hugging Face AutoModelForCausalLM object for scoring. """
    def __init__(self, model: PreTrainedModel, tokenizer, prompt_template: str, **kwargs):
        self.model = model
        self.tokenizer = tokenizer
        self.prompt_template = prompt_template
        # Hugging Face models use GenerationConfig
    ...
    def parse_score(self, response_text: str) -> float:
        if "<score>REFUSAL</score>" in response_text: return None
        match = re.search(r"<score>(\d+)</score>", response_text)
        if match:
